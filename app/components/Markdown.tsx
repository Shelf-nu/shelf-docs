...
const CustomLink = (props) => {
	...
	if (isInternalLink) {
	  return (
		<Link style={{color:"#EE6821"}} to={href} prefetch="intent" {...props}>{props.children}</Link>
	  );
	}
  
	return <a style={{color:"#EE6821"}} target="_blank" rel="noopener noreferrer" {...props} />;
};
...
export function QuickLink({ title, description, href }: QuickLinkProps) {
	return (
		<div className="group relative rounded-xl border border-slate-200 dark:border-slate-800" >
			...
			<div  className="absolute -inset-px rounded-xl border-2 border-transparent opacity-0 [background:linear-gradient(var(--quick-links-hover-bg,theme(colors.blue.50)),var(--quick-links-hover-bg,theme(colors.blue.50)))_padding-box,linear-gradient(to_top,theme(colors.red.400),theme(colors.pink.400),theme(colors.orange.500))_border-box] group-hover:opacity-100 dark:[--quick-links-hover-bg:theme(colors.slate.800)]" />
